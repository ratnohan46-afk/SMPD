...
async function fetchPlayersForServer(server, timeoutMs) {
  ...
  try {
    ...
    if (playersUrl.startsWith("http://")) {
      try {
        const httpsUrl = playersUrl.replace(/^http:\/\//i, "https://");
        ...
        // Keep discovery data when players.json is unavailable.
      }
    }
    ...
    return {
      ...
      playerFetchError: firstError?.message || "players.json unavailable"
    };
  }
}
...
async function enrichPlayers(
  servers,
  { timeout = 10000, concurrency = 8, delayMs = 100 } = {}
) {
...
export async function discoverServers({
  ...
  return enrichPlayers(result, {
    timeout: Math.max(
      3000,
      Math.min(Number(timeout) || 10000, 15000)
    ),
    concurrency: 8,
    delayMs
  });
...
export function findPlayers(servers, term) {
  ...
  for (const server of servers ?? []) {
    for (const player of server.players ?? []) {
      if (!String(player.name).toLocaleLowerCase().includes(query)) {
        continue;
      }
